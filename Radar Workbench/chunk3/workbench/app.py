"""OpenFlight Radar Radar Workbench -- offline pages.
...
from openflight_bench.analysis import (  # noqa: E402
    CollectingProgress, ConfigurationPolicy, MaterialOptions, SweepOptions,
...
def page_analyze(directory: str, rb):
    st.header("Analyze")
    st.caption("One capture on its own.")
    ...
    a = analyze_capture(cap, guard_bins=guard)

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("peak bin", a.peak_bin)
    c2.metric("peak range", f"{a.peak_range_m * plots.M_TO_IN:.1f} in")
    c3.metric("SNR", f"{a.snr_db:.1f} dB")
    c4.metric("noise floor", f"{a.noise_db:.1f} dB")
    st.write(_integrity_badge(cap), "·", cap.integrity.describe_frames())
    for w in cap.warnings:
        st.warning(w)

    st.pyplot(plots.range_profile(a), use_container_width=True)
    left, right = st.columns(2)
    left.pyplot(plots.element_heatmap(a), use_container_width=True)
    right.pyplot(plots.element_heatmap(a, quantity="phase"), use_container_width=True)
