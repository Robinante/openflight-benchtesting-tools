...
def analyze_capture(
    capture, *, gate=None, guard_bins: int = 2, forced_peak_bin: int | None = None,
    extra_noise_exclude_bins: set[int] | None = None, strict_noise: bool = False,
    policy: ConfigurationPolicy | None = None, annotate: bool = True,
    min_prominence_db: float = 3.0,
):
    """Open one capture raw-first and return a :class:`CaptureAnalysis`.

    The full range profile is computed and attached FIRST, before any target
    decision, and is always present. Peak detection and the target-derived
    metrics are annotation layered on top; if they fail, the reason is recorded
    in ``annotation_error`` and the profile is still returned.

    ``gate`` is optional and explicit. Supplying one adds region metrics under
    ``gate_metrics``; it never filters, masks or replaces ``profile.power``.
    ``annotate=False`` skips peak detection entirely -- pure raw output.
    """
    from .rangeprofile import Gate, detect_peaks, range_profile   # local: cycle
    from .results import CaptureAnalysis
    from . import metrics as _m

    cap = as_capture(capture, policy=policy)
    profile = range_profile(cap)          # <-- no selection, cannot be gated away

    analysis = CaptureAnalysis(
        captures=[cap], capture=cap, profile=profile,
        start_bin=profile.start_bin, power_profile=profile.power,
        gate=gate,
        peak_bin=None, peak_range_m=None, noise_db=None, snr_db=None,
    )

    if annotate:
        try:
            analysis.peaks = detect_peaks(profile, min_prominence_db=min_prominence_db)
            analysis.noise_floor_db = _m.noise_floor_db(
                profile, peaks=analysis.peaks, guard_bins=guard_bins)
            analysis.peak_to_floor_db = _m.peak_to_floor_db(
                profile, peaks=analysis.peaks, guard_bins=guard_bins)
        except Exception as exc:                      # noqa: BLE001
            analysis.annotation_error = f"peak detection: {type(exc).__name__}: {exc}"

    # Legacy element extraction -- the coherent per-(TX,RX) means at one bin.
    # Wrapped so a target-selection failure costs the annotation, not the data.
    try:
        ...
        analysis.peak_bin = r["peak_bin"]
        analysis.peak_range_m = r["peak_range_m"]
        analysis.noise_db = r["noise_db"]
        analysis.snr_db = r["snr_db"]
        analysis.element_mean = r["element_mean"]
        analysis.coherence = r["coherence"]
        analysis.n_frames_used = r["n_frames_used"]
        analysis.n_loops_total = r["n_loops_total"]
        analysis.coherent_gain_db = _m.coherent_gain_db(
            r["element_mean"], profile, r["peak_bin"])
    except Exception as exc:                          # noqa: BLE001
        prev = analysis.annotation_error
        msg = f"element extraction: {type(exc).__name__}: {exc}"
        analysis.annotation_error = f"{prev}; {msg}" if prev else msg

    if gate is not None:
        try:
            vals = profile.within(gate)
            lo_m, hi_m = gate.range_bounds_m(profile)
            analysis.gate_metrics = {
                "gate_lo_bin": gate.lo_bin, "gate_hi_bin": gate.hi_bin,
                "gate_label": gate.label, "gate_lo_m": lo_m, "gate_hi_m": hi_m,
                "n_bins_in_gate": int(vals.size),
                "mean_power_db_in_gate": _m.mean_power_db(profile, gate),
                "peak_power_db_in_gate": _m.power_db(vals.max()) if vals.size else None,
                "peak_bin_in_gate": (int(gate.lo_bin + int(vals.argmax()))
                                     if vals.size else None),
            }
            if analysis.noise_floor_db is not None and vals.size:
                analysis.gate_metrics["peak_to_floor_db_in_gate"] = float(
                    _m.power_db(vals.max()) - analysis.noise_floor_db)
        except Exception as exc:                      # noqa: BLE001
            analysis.gate_metrics = {"error": f"{type(exc).__name__}: {exc}"}
