...
def raw_range_profile(analysis, *, units="in", gate=None, annotate_peaks=True,
                      max_peak_labels=4, zoom=False, ax=None, title=None):
    """The full captured range profile, raw first.

    Every captured bin is drawn, always. Peaks are drawn as annotation markers
    and a gate as a shaded span with dashed bounds -- neither removes data from
    the plot. ``zoom=True`` is the only thing that restricts the x-axis, and it
    is opt-in: by default you see the whole window even when a gate is set.
    """
    plt = _plt()
    if ax is None:
    ...
    scale = M_TO_IN if units == "in" else 1.0
    x = prof.range_m * scale
    db = prof.power_db

    if gate is not None:
        lo_m, hi_m = gate.range_bounds_m(prof)
        half = 0.5 * prof.range_bin_m * scale
        ax.axvspan(lo_m * scale - half, hi_m * scale + half,
                   color=GREEN, alpha=0.12, zorder=0)
        for edge in (lo_m * scale - half, hi_m * scale + half):
            ax.axvline(edge, color=GREEN, ls="--", lw=1.2, zorder=2)
        ax.text(((lo_m + hi_m) / 2) * scale, db.min(),
                f"gate {gate.label or ''}".strip(), ha="center", va="bottom",
                fontsize=8.5, color=GREEN, zorder=6)

    ax.plot(x, db, lw=1.6, color=BLUE, zorder=3)

    if analysis.noise_floor_db is not None:
        ax.axhline(analysis.noise_floor_db, color=MUTED, ls="--", lw=1.0, zorder=1)
        ax.text(x[0], analysis.noise_floor_db, " no-return floor", fontsize=8,
                color="#52514e", va="bottom")

    if annotate_peaks and analysis.peaks:
        for pk in analysis.peaks[:max_peak_labels]:
            px = pk.range_m * scale
            inside = gate is not None and gate.contains_bin(pk.bin)
            ax.plot([px], [pk.power_db], "o", ms=9,
                    color=GREEN if inside else ORANGE, mec=SURF, mew=1.5, zorder=5)
            ax.annotate(f"bin {pk.bin}\n+{pk.prominence_db:.1f} dB",
                        (px, pk.power_db), textcoords="offset points",
                        xytext=(0, 9), ha="center", fontsize=7.8, color="#52514e")

    if zoom and gate is not None:
        lo_m, hi_m = gate.range_bounds_m(prof)
        pad = 4 * prof.range_bin_m * scale
        ax.set_xlim(lo_m * scale - pad, hi_m * scale + pad)

    lo_db, hi_db = float(db.min()), float(db.max())
    span = max(hi_db - lo_db, 1.0)
    ax.set_ylim(lo_db - 0.08 * span, hi_db + 0.22 * span)
    ax.set_xlabel(f"range ({'inches' if units == 'in' else 'meters'})")
    ax.set_ylabel("power (dB re 1 ADC count squared)")
    n = prof.n_bins
    head = title or (f"{prof.source_name}  ·  all {n} captured bins"
                     + (f"  ·  {len(analysis.peaks)} return(s) detected"
                        if analysis.peaks else ""))
    ax.set_title(head, loc="left", color=INK, fontsize=11.5, pad=8)
    return _tidy(ax).figure
